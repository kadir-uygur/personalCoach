$(document).ready(function () {
    ...
    function showResults(calories) {
        results.innerHTML = `<p>Ortalama Aktivite Derecenize Göre Günlük Kalori İhtiyacınız : <strong>${calories.toFixed(2)} </strong></p><a href="#" id="rs">tekrar hesapla</a>`;
    ...
    function submitHandler(e) {
        ...
        if (isNaN(age) || age < 0) {
            return errorMessage('Yaşınızı giriniz');
        }
        ...
        if (isNaN(heightCM) || heightCM < 0) {
            ...
            if (isNaN(heightFeet) || heightFeet < 0) {
                return errorMessage('Boyunuzu giriniz');
            }
        ...
        if (form.gender.value == 'Female') {
            ...
            calories = (655.1 + (9.56 * weight) + (1.85 * heightCM )- (4.67 * age)) * 1.55;
        } else {
            calories = (66.5 + (13.75 * weight) + (5 * heightCM) - (6.77 * age)) * 1.55;
        }
